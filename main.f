*deck main
	program main
      ...
      parameter(imt=142,jmt=142,imm=imt-1,imm1=imm-1,jmm=jmt-1,
     *     jmm1=jmm-1,kmt=120)
...
c        write (203,9900) (ua(i,j),i=2,imm)
c        write (203,9900) (v(i,j),i=2,imm)
 250	continue
...
c	do 7 j=js,jn
c        do 7 i=iwe,iea
c              ddphj=pi*float(j-js)/float(jn-js)
c              ddphi=pi*float(i-iwe)/float(iea-iwe)
c 7	   dh0(i,j)=ddh*sin(ddphi)*sin(ddphj)
	do 8 j=1,jmt
        do 8 i=1,imt
